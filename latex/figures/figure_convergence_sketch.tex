	...
	\makebox[\textwidth][c]{
		...
		\begin{tikzpicture}[font=\scriptsize, align=center]
				\node[draw, above left, align=center] (a) {
					...
					$\begin{cases}
						\text{Min~} & \frac{nu \cdot L}{2} \sum_{k=1}^{L} \norm{v_k}_\cV^2
						+ l_R(\Phi_L(X), Y) \\
						\text{s.t.~} & v_1, \ldots, v_L \in \cV,\ \Phi_L = (I + v_L) \circ \ldots \circ (I + v_1)
					\end{cases}$};
				\node[right=of a, align=center] (z1) {\large$\stackrel{L \rightarrow \infty}{\longrightarrow}$\\
				\scriptsize\cref{theo:problem-convergence}};
				\node[draw, right= of z1, align=center] (b) {
					Continuous Limit of ResNet \ref{prob:resnet-limit}\\
					$\begin{cases}
				...
				\node[draw, below= of z2, align=center] (c) {
					...
					$\begin{cases}
						\text{Min~} & \frac{\nu}{2} \sum_{k=1}^{L} \left(\frac{q_{k+1} - q_k}{\Delta t}\right)^\mathrm{T} \bGamma(q_k, q_k)^{-1} \left(\frac{q_{k+1} - q_k}{\Delta t}\right) \Delta t+ l(q_{L+1}, Y) \\
				...
				\node[right=of c, align=center] (z4) {\large$\stackrel{L \rightarrow \infty}{\longrightarrow}$\\
				\scriptsize\cref{theo:problem-convergence}};
				...
				\node[draw, below= of z5, align=center] (e) {
					Geodesic Shooting \ref{eq:p0-min-function}\\
					$\begin{cases}
