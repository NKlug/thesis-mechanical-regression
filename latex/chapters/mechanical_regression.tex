...
\begin{problem}
	\label{prob:min-q}
	\begin{cases}
		\text{Minimize~} & \nu \cdot \frac{1}{2} \sum_{k=1}^{L} \frac{(q_{k+1} - q_k)^\mathrm{T}}{\Delta t} \bGamma(q_k, q_k)^{-1} \left(\frac{q_{k+1} - q_k}{\Delta t}\right) \Delta t+ l(q_{L+1}, Y) \\
		\text{such that~} & q_1 = X \text{~and~} q_2, \ldots, q_{L+1} \in \cX^N \ .
	\end{cases}
\end{problem}
\Todo{Explain what it means for problems to be equivalent!}
...
\begin{theorem}
	...
	\label{eq:hamiltonian-system}
		\begin{split}
			\dot{q} &= \frac{\partial \fH(q, p)}{\partial p} = \bGamma(q, q) p\\
...
\label{theo:geodesic-shooting}
	$q \in C^1([0, 1], \cX^N)$ minimizes \cref{prob:cont-least-action} if and only if with $p\coloneqq \bGamma(q, q)^{-1}\dot{q}$, $(q, p)$ satisfy Hamilton's equations \ref{eq:hamiltonian-system}, $q(0) = X$ and $p(0)$ minimizes
\begin{equation}
	\label{eq:p0-min-function}
	\mathfrak{V}(p(0), X, Y) \coloneqq  \frac{\nu}{2} p(0)^\T \bGamma(X, X)p(0) + l(q(1), Y)
\end{equation}
\end{theorem}
\begin{proof}
	Sketch (for now):
