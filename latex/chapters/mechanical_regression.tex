...
\begin{theorem}
$q \in C^1([0, 1], \cX^N)$ minimizes \cref{prob:cont-least-action} if and only if with $p\coloneqq \mathbf{\Gamma}^{-1}(q, q)\dot{q}$, $(q, p)$ satisfy Hamilton's equations \ref{eq:hamiltonian-system}, $q(0) = X$ and $p(0)$ minimizes
\begin{equation}
...
\begin{proof}
	...
	\end{equation}
	for $\beta \in [0, 1]$.
	Therefore $(q, p)$ minimize $\nu \cH(0, q, p) + l(q(1), Y)$ and for $\beta = 0$ we get
	\begin{align}
		\nu \cH(0, q, p) + l(q(1), Y) &= \frac{\nu}{2} p(0)^\T \mathbf{\Gamma}(q(0), q(0))p(0) + l(q(1), Y)\\
		&= \frac{\nu}{2} p(0)^\T \mathbf{\Gamma}(X, X)p(0) + l(q(1), Y) \ .
	\end{align}
