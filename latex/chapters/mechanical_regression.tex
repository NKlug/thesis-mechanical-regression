...
\begin{proof}
	...
	\begin{align}
		...
		&= \norm{\sum_{i=1}^N \Gamma(x, q_i)^\T p_i}_\cV^2\\
		&= \norm{\sum_{i=1}^N \Gamma(q_i, x) p_i}_\cV^2\\
		&= \sum_{i=1}^N \sum_{j=1}^N \left<\Gamma(q_i, x)p_i, \Gamma(q_j, x) \psi(q_j)p_i \right>_\cV\\
	\end{align}
\end{proof}
