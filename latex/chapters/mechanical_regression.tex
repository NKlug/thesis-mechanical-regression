...
The obtained $(q, p)$ are called the \emph{canonical variables}, concretely canonical coordinate and canonical momentum.
...
However, treating them separately will bring us no benefit, so we again just treat them as one and call $p$ \emph{the} canonical momentum.
Next, define the \emph{Hamiltonian function}.
\begin{equation}
	\fH(q, p) = p^\mathrm{T}\dot{q} - \fL(q, \dot{q})
\end{equation}
\begin{theorem}
	Any minimizer $q$ of \cref{eq:cont-least-action} follows the Hamiltonian equations
	\begin{align}
