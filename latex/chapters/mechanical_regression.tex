...
\begin{condition}
	\begin{enumerate}
		\item There exist a finite dimensional feature space and map $\mathcal{F}$ and $\psi$ such that $\psi$ and its first and second order partial derivatives are continuous and uniformly bounded
	\end{enumerate}
\end{condition}
...
\begin{proof}
	...
	This gives us $\norm{\alpha}_\mathcal{F}^2 = 2 \fH(q, p)$.
	\cref{cor:energy-preservation} states that $\fH$ is constant across time, which implies that $\norm{\alpha}_\mathcal{F}^2$ is, too.
	From \cref{eq:feature-hamiltonian} we first get
	\begin{align}
		\norm{\dot{p_i}(t)}_\cX &= \norm{-\grad_{q_i}{\left(p_i^\mathrm{T} \psi^\mathrm{T}(q_i) \alpha \right)}}_\cX\\
		&= \norm{-\grad_{q_i}{\left<p_i(t), \psi^\mathrm{T}(q_i(t)) \alpha \right>}}_\cX\\
		&= \norm{\left<p_i(t), \frac{\partial}{\partial q_i} \left(\psi^\mathrm{T}(q_i(t)) \alpha \right)\right>}_\cX\\
		&\leq \norm{p_i(t)}_\cX \norm{\frac{\partial}{\partial q_i} \left(\psi^\mathrm{T}(q_i(t)) \alpha \right)}_\cX\\
	\end{align}
