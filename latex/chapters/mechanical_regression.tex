...
%	\end{align}
	...
	From that section for the minimal $v_k$ we conveniently get the following representations for the minimal $v_k$ and the target value:
	\begin{align}
	...
	\begin{equation}
		\begin{cases}
			...
			\frac{(q_{k+1} - q_k)^\mathrm{T}}{\Delta t} \mathbf{\Gamma}(q_k, q_k)
			\frac{(q_{k+1} - q_k)}{\Delta t} \cdot \Delta t
			+ l(q_{L+1}, Y) \\
			\text{such that~} & q_1 = X \text{~and~} q_2, \ldots, q_{L+1} \in \cX^N \ .
		\end{cases}
	\end{equation}
