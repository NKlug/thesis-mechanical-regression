...
\begin{align}
	...
	q_{n+1} &\gets q_n + h \grad_q\fH\left(q_n, p_{n+\frac{1}{2}}\right)\\
...
\begin{equation}
q_{n+1} \gets q + \frac{h}{2} \left(\grad_q\fH\left(q_n, p_{n+\frac{1}{2}}\right) + \grad_q\fH\left(q_{n+1}, p_{n+\frac{1}{2}}\right)\right) \ .
\end{equation}
...
In order to fully make use of modern tensor-processing frameworks' parallel processing abilities, expressing the scalar product of $\cX^N$ (which is defined as $\sum_{i=1}^N \left<x_{1, i}, x_{2, i}\right>$ for $x_1, x_2 \in \cX^N$) in terms of matrix operations is highly desirable.
...
One such place is the gradient of $\cH$ with respect to $q$.
